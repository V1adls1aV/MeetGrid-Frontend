...
describe('Voting calendar — mobile', () => {
  it('collapses resources and keeps calendar scrollable', () => {
    cy.viewport(390, 844);
    cy.intercept('GET', '**/api/v1/topic/demo?username=mobile', topicResponse).as('fetchTopic');
    cy.visit('/topic/demo', {
      onBeforeLoad(win) {
    ...
    cy.get('.rbc-row-resource').should('have.length', 2).then(($cells) => {
