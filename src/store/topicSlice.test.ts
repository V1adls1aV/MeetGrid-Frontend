...
describe("topicSlice form draft", () => {
  it("merges draft form updates without dropping untouched fields", () => {
    const stateWithTopic = topicReducer(
      baseState(),
      setDraftForm({ topicName: "Retro" }),
    );
    const stateWithAdmin = topicReducer(
      stateWithTopic,
      setDraftForm({ adminName: "Влад" }),
    );

    expect(stateWithAdmin.draftForm).toEqual({
      ...emptyForm,
      topicName: "Retro",
      adminName: "Влад",
    });
